...
# Repositories excluded from language statistics.
EXCLUDED_REPOS = {
    USERNAME,  # Exclude the profile README repository.
}

# Languages excluded from language statistics.
EXCLUDED_LANGUAGES = {
    # "TeX",
    # "HTML",
}
...
def get_contribution_activity():
    ...
    A day is considered active when GitHub reports contributionCount > 0.
    The longest streak is therefore the longest consecutive run of active
    calendar days inside this same 12-month window.
    """
    ...
    # Ensure chronological order before computing streaks.
...
"""
    ...
    # The bar displays only the selected top-N languages, normalized to their
    # combined total so that the bar spans the full card width.
...
"""
    ...
    # Percentages remain relative to all included language bytes, not just top N.
